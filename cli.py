...
from config import EARLIEST_ALLOWED_DATE, LATEST_ALLOWED_DATE
from db import (
    create_indices,
    create_tables,
    create_views,
...
from processor import post_process_data, process_13f_data
...
def reset(
    ...
    if not confirm:
        console.print(
            ":warning: Warning: This action will permanently delete all data in the database.",
            style="bold yellow",
        )
...
@app.command("d")
def download(
    start_date: str = typer.Argument(..., help="Start date (YYYY-Qi format)"),
    end_date: str = typer.Argument(..., help="End date (YYYY-Qi format)"),
):
    """Download 13F data
...
def postprocess():
    ...
    console.print("FTD data downloaded and processed. :thumbs_up:", style="bold green")
    post_process_data()
